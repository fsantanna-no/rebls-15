...
recursive block. It is designed to take into account the event system and
...
specific memory pool. Therefore, for bounded pools (e.g., \code{List[3] lst}),
...
To illustrate these differences, \emph{CODE-1} in 
Figure~\ref{lst.list.sum.react} extends the body of the previous example with 
reactive behavior.
For each recursive iteration, \code{traverse} prints the current 
\code{head} (line 9) and awaits 1 second before traversing the \code{tail} 
(lines 10--11).
In \CEU, all accesses to a pointer that cross \code{await} statements must be 
protected with an enclosing \code{watching} block~\cite{ceu.mod15}, which 
aborts its nested block if the referred object is released from memory.
   ...
   watching this.parent do
      if e:NIL then
         ...
         watching e do
            _printf("me = %d\n", e:CONS.head);
            await 1s;
            var Body* body = spawn Body(this.bodies,
                                        &this,
                                        &e:CONS.tail);
                             in this.bodies;
            var int sum_tail = await *body;
            escape sum_tail + e:CONS.head;
         end
         escape 0;
      end
...
pool Body[3] bodies;
var Body* body = spawn Body(bodies, &this, &lst)
                 in bodies;
var int sum = await *body;
...
TODO: option type, mem fail, passing arguments, outer closure
...
% number of steps, each of them in a separate synchronous reaction. (TODO: not 
% sure what you mean...)
...
TODO:
given the structured reasoning and the fact that the languages forces the 
programmer to write the appropriate watching,
we believe it is easy to use
hope to prove in the following section
...
In this section, we present two applications that explore the reactive and 
incremental nature of the \code{traverse} construct.
We start with standard \emph{Behavior Trees} used in video games for AI 
modeling~\cite{isla2005}~\cite{hecker2009my}.
Then, we show a \emph{Logo Turtle}~\cite{papert.logo} that can execute commands in 
...
\end{lstlisting}
%\rule{8.5cm}{0.37pt}
\caption{ A simple generic grammar of behavior trees with \emph{sequence} and \emph{selector}
          composite nodes.
\label{lst.bt1}
}
\end{figure}
...
\end{lstlisting}
%\rule{8.5cm}{0.37pt}
\caption{ A straightforward interpreter for the behavior tree of 
          Figure~\ref{lst.bt1}.

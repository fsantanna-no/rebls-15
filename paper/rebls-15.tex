...
reactive behavior: incremental computation and control-oriented domain specific 
languages.
...
\begin{itemize}
    ...
    \item gray binary gen (incremental)
\end{itemize}

I suggest gray binary as the last one because it does not use a data type.
...
TODO: small discussion incremental / DSLs and then proceed to each example?
...
\subsubsection{TODO: Standard Behavior Tree}
...
a data type which defines our abstract syntax, which each tag representing one
of the supported Logo commands. A tree of nodes represents a program, and
...
\subsubsection{Command Streaming}

\subsection{Gray Binary Generation}
...
\end{lstlisting}
%\rule{8.5cm}{0.37pt}
\caption{ Gray binary generation for tuples of size 4.
\label{lst.gray}
}
\end{figure}

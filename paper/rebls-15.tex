...
\CEU supports mutable data structures with static memory and safe pointer 
manipulation.
...
that supports trees (but not more general graphs).
To manage reactive behavior, we propose a structured mechanism that can 
traverse data types safely and incrementally, in successive reactions to input 
events.
%\code{traverse} is an iterator-like anonymous block that can be invoked 
%recursively and suspended at any point, retaining the full state and stack 
%frames alive.
...
in the domain control-oriented DSLs.
...
and receives the tree to traverse as the argument (line 12).
The body acquires the return status of the traversal (line 14) and returns it 
as its final result (lines 36).
%
...
The tree traversal hangs and waits for its return status (line 33), which is 
...
\code{waiting} items.
%
\item Lines 10--11 assign the \code{waiting} subtree (mark~\MM(B)) to the 
\code{running} field, releasing the old subtree (mark~\MM{(A)})).
...
\code{ITEM} in the root of the \code{waiting} field (mark~\MM{(D)}).
%
\item Lines 4--11 assigns a new subtree to the \code{tmp} field (box~\code{1}) 
with a new neutral \code{ITEM} (mark~\MM{(E)}, lines 5--6) linked to the set of 
commands to \code{MOVE} the turtle (mark~\MM{(F)}, lines 7--11).
%
\item Lines 15--16 move the already \code{waiting} commands (mark~\MM{(G)}) to 
tail of the \code{tmp} node (mark~\MM{(H)}).
...
%
\item Line 17 moves the \code{tmp} subtree (mark~\code{E}) back to the 

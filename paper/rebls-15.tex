...
The second tag, \code{CONS} (ln. 4), receives two arguments in the constructor: 
...
%
All instances of a recursive data type must reside in an explicit memory pool, 
which have static memory management based on its enclosing lexical scope (ln.  
7--16).
%
...
Also, elements in different pools cannot be mixed without copying.
...
traversal %using the language's concurrency features
...
%abortion via the \code{watching} construct.
